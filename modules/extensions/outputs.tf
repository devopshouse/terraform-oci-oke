...
resource "null_resource" "service_account_token_retrieve_crb" {
  ...
  triggers = {
    ...
    # Force update when code changes - using permanent secret-based token method
    token_method = "permanent_secret_token_v1"
    # Force refresh on every apply to ensure token is always up-to-date (idempotent)
    # This ensures the token in state is always the current token from the secret
